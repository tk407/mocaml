...
with Get : G -> expr -> typexpr -> Prop :=    (* defn Get *)
 ...
 | Get_live_comp : forall (G5:G),
     Get G5 (E_live_expr LM_comp) (TE_concurrent (TE_constants TC_unit))
...
(* defns Jop *)
Inductive JO_red : expr -> select -> expr -> Prop :=    (* defn red *)
 | JO_red_app : forall (x:value_name) (t:typexpr) (e:expr) (s:select) (v:expr),
     is_value_of_expr v ->
     JO_red (E_apply  (E_function x t e)  v) s  (subst_expr  v   x   e ) 
 | JO_red_forkmove1 : forall (e e' e'':expr) (s:select),
     ~ ( is_value_of_expr ( e' ) )  ->
     JO_red e s e'' ->
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr (LM_expr e)) )   (E_live_expr (LM_expr e')) ) S_First (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr (LM_expr e'')) )   (E_live_expr (LM_expr e')) )
 | JO_red_forkmove2 : forall (e e' e'':expr) (s:select),
     ~ ( is_value_of_expr ( e ) )  ->
     JO_red e' s e'' ->
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr (LM_expr e)) )   (E_live_expr (LM_expr e')) ) S_Second (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr (LM_expr e)) )   (E_live_expr (LM_expr e'')) )
 | JO_red_forkdeath1 : forall (v e:expr) (s:select),
     is_value_of_expr v ->
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr (LM_expr v)) )   (E_live_expr (LM_expr e)) ) s (E_live_expr (LM_expr  (E_taggingleft  (E_taggingright  (E_pair v  (E_live_expr (LM_expr e)) ) ) ) ))
 | JO_red_forkdeath2 : forall (e v':expr) (s:select),
     is_value_of_expr v' ->
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr (LM_expr e)) )   (E_live_expr (LM_expr v')) ) s (E_live_expr (LM_expr  (E_taggingright  (E_pair  (E_live_expr (LM_expr e))  v') ) ))
 | JO_red_forkdeath12 : forall (v v':expr) (s:select),
     is_value_of_expr v ->
     is_value_of_expr v' ->
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr (LM_expr v)) )   (E_live_expr (LM_expr v')) ) s (E_live_expr (LM_expr  (E_taggingleft  (E_taggingleft  (E_pair v v') ) ) ))
 | JO_red_forkdocomp1 : forall (e:expr) (s:select),
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr LM_comp) )   (E_live_expr (LM_expr e)) ) s (E_live_expr (LM_expr  (E_taggingleft  (E_taggingright  (E_pair (E_constant CONST_unit)  (E_live_expr (LM_expr e)) ) ) ) ))
 | JO_red_forkdocomp2 : forall (e:expr) (s:select),
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr (LM_expr e)) )   (E_live_expr LM_comp) ) s (E_live_expr (LM_expr  (E_taggingright  (E_pair  (E_live_expr (LM_expr e))  (E_constant CONST_unit)) ) ))
 | JO_red_forkdocomp12 : forall (s:select),
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr LM_comp) )   (E_live_expr LM_comp) ) s (E_live_expr (LM_expr  (E_taggingleft  (E_taggingleft  (E_pair (E_constant CONST_unit) (E_constant CONST_unit)) ) ) ))
 | JO_red_ret : forall (v:expr) (s:select),
     is_value_of_expr v ->
     JO_red (E_apply (E_constant CONST_ret) v) s  (E_live_expr (LM_expr v)) 
 | JO_red_evalbind : forall (e e'':expr) (s:select) (e':expr),
     JO_red e s e' ->
     JO_red (E_bind e e'') s (E_bind e' e'')
 | JO_red_movebind : forall (e e'':expr) (s:select) (e':expr),
     JO_red e s e' ->
     JO_red (E_bind  (E_live_expr (LM_expr e))  e'') s (E_bind  (E_live_expr (LM_expr e'))  e'')
 | JO_red_compbind : forall (e:expr) (s:select) (e':expr),
     JO_red (E_bind  (E_live_expr LM_comp)  e) s (E_apply e' (E_constant CONST_unit))
 | JO_red_dobind : forall (v e:expr) (s:select),
     is_value_of_expr v ->
     JO_red (E_bind  (E_live_expr (LM_expr v))  e) s (E_apply e v)
 | JO_red_context_app1 : forall (e e':expr) (s:select) (e'':expr),
     JO_red e' s e'' ->
     JO_red (E_apply e e') s (E_apply e e'')
 | JO_red_context_app2 : forall (e v:expr) (s:select) (e':expr),
     is_value_of_expr v ->
     JO_red e s e' ->
     JO_red (E_apply e v) s (E_apply e' v)
 | JO_red_fix_move : forall (e:expr) (s:select) (e':expr),
     JO_red e s e' ->
     JO_red  (E_fix e)  s  (E_fix e') 
 | JO_red_fix_app : forall (v v':expr) (s:select),
     is_value_of_expr v ->
     is_value_of_expr v' ->
     JO_red (E_apply  (E_fix v)  v') s (E_apply  (E_fix v)   (E_apply v v') )
 | JO_red_pair_1 : forall (e e'':expr) (s:select) (e':expr),
     JO_red e s e' ->
     JO_red (E_pair e e'') s (E_pair e' e'')
 | JO_red_pair_2 : forall (v e:expr) (s:select) (e':expr),
     is_value_of_expr v ->
     JO_red e s e' ->
     JO_red (E_pair v e) s (E_pair v e')
 | JO_red_evalinl : forall (e:expr) (s:select) (e':expr),
     JO_red e s e' ->
     JO_red (E_taggingleft e) s (E_taggingleft e')
 | JO_red_evalinr : forall (e:expr) (s:select) (e':expr),
     JO_red e s e' ->
     JO_red (E_taggingright e) s (E_taggingright e')
 | JO_red_evalcaseinl : forall (x:value_name) (e:expr) (x':value_name) (e':expr) (s:select) (v:expr),
     is_value_of_expr v ->
     JO_red (E_case  (E_taggingleft v)  x e x' e') s  (subst_expr  v   x   e ) 
 | JO_red_evalcaseinr : forall (x:value_name) (e:expr) (x':value_name) (e':expr) (s:select) (v:expr),
     is_value_of_expr v ->
     JO_red (E_case  (E_taggingright v)  x e x' e') s  (subst_expr  v   x'   e' ) 
 | JO_red_evalcase : forall (e:expr) (x:value_name) (e'':expr) (x':value_name) (e''':expr) (s:select) (e':expr),
     JO_red e s e' ->
     JO_red (E_case e x e'' x' e''') s (E_case e' x e'' x' e''').
