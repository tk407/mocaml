...
Extract Constant select => "(select ())".
...
(** subrules *)
Fixpoint xis_expr_of_expr (e_5:expr) : bool :=
  match e_5 with
  | (E_ident value_name5) => (true)
  | (E_constant constant5) => (true)
  | (E_apply expr5 expr') => (if (xis_expr_of_expr expr5) then (xis_expr_of_expr expr') else false)
  | (E_bind expr5 expr') => (if (xis_expr_of_expr expr5) then (xis_expr_of_expr expr') else false)
  | (E_function value_name5 expr5) => ((xis_expr_of_expr expr5))
  | (E_live_expr expr5) => ((xis_expr_of_expr expr5))
  | (E_dead_expr value5) => ((xis_value_of_expr value5))
  | (E_pair e e') => (if (xis_expr_of_expr e) then (xis_expr_of_expr e') else false)
  | (E_taggingleft e) => ((xis_expr_of_expr e))
  | (E_taggingright e) => ((xis_expr_of_expr e))
  | (E_case e1 x1 e2 x2 e3) => (if (xis_expr_of_expr e1) then ( if (xis_expr_of_expr e2) then (xis_expr_of_expr e3) else false) else false)
end
with xis_value_of_expr (e_5:expr) : bool :=
  match e_5 with
  | (E_ident value_name5) => false
  | (E_constant constant5) => (true)
  | (E_apply expr5 expr') => false
...
Fixpoint yexp (e : expr) (b : bool) : Prop := (eq (xis_expr_of_expr e) b).
...
Fixpoint subst_expr (e_5:expr) (x_5:value_name) (e_6:expr) {struct e_6} : expr :=
  match e_6 with
...
Inductive JO_red : expr -> expr -> Prop :=    (* defn red *)
 ...
 | JO_red_forkmove2 : forall (e e' e'':expr),
     ...
     JO_red e' e'' ->
     JO_red (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr e) )   (E_live_expr e') ) (E_apply  (E_apply (E_constant CONST_fork)  (E_live_expr e) )   (E_live_expr e'') )
(* | JO_red_forkmove12 : forall (e e' e'' e''':expr),
     (eq (xis_value_of_expr e) false) ->
     (eq (xis_value_of_expr e') false) ->
     (eq select Both) ->
     JO_red e e'' ->
...
Extraction Relation Relaxed JO_red [1].