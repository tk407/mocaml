...
(* 
 ...
 | XJO_red_evalret : forall (v v':expr) (s:selectstar),
 ...
 | XJO_red_inpair_eval2 : forall (v v' v'' :expr) (s:selectstar),
     (eq (xis_value_of_expr v) true) ->
     (eq (xis_value_of_expr v') false) ->
     XJO_red v' s v'' ->
     XJO_red (E_inpair v  v') s (E_inpair v v'')  
 | XJO_red_inpair_eval1 : forall (v v' v'' :expr) (s:selectstar),
     (eq (xis_value_of_expr v) false) ->
     XJO_red v s v'' ->
     XJO_red (E_inpair v  v') s (E_inpair v'' v') 
 ...
 | XJO_red_proj1_eval : forall (v v':expr) (s:selectstar),
     (eq (xis_value_of_expr v) false) ->
     XJO_red v s v' -> 
 ...
 | XJO_red_proj2_eval : forall (v v':expr) (s:selectstar),
     (eq (xis_value_of_expr v) false) ->
     XJO_red v s v' -> 
     XJO_red (E_proj2 v) s  (E_proj2 v') *)
 
 (*| XJO_red_forkeval1 : forall (e e':expr) (s:selectstar) (rl:redlabel) (e'':expr),
     (eq (xis_value_of_expr e) false) ->
     XJO_red e s e'' ->
     XJO_red (E_fork e e') s (E_fork e'' e')
 | XJO_red_forkeval2 : forall (v e':expr) (s:selectstar) (rl:redlabel) (e'':expr),
     (eq (xis_value_of_expr v) true) ->
     (eq (xis_value_of_expr e') false) ->
     XJO_red e' s e'' ->
 ...
 | XJO_red_forkmove1 : forall (e e':expr)  (e'':expr) (s:selectstar),
     ...
     (eq (xis_value_of_expr e') false) ->
     XJO_red e s e'' ->
     XJO_red (E_fork  (E_live_expr e)   (E_live_expr e') ) (Seq S_First s) (E_fork  (E_live_expr e'')   (E_live_expr e') )
 | XJO_red_forkmove2 : forall (e e':expr)  (e'':expr) (s:selectstar),
     (eq (xis_value_of_expr e) false) ->
     (eq (xis_value_of_expr e') false) ->
     XJO_red e' s e'' ->
     XJO_red (E_fork  (E_live_expr e)  (E_live_expr e') ) (Seq S_Second s) (E_fork  (E_live_expr e)    (E_live_expr e'') )
