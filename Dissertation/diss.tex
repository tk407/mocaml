...
The statement \lstinline[language={Ott}]|single t x :: tsubsts| in \Cref{lst:ottsubstex} defines a single substitution function called tsubsts\_t over terms defined by the grammar for t and for variables represented by the metavariable x. This is the same tsubst\_t mentioned \Cref{lst:ottgrammarex}.
...
In \Cref{lst:ottredex} I define a set of mutually recursive relations named Jop with one relation in it the $\longrightarrow$ or reduce relation. Each element of this relation takes the form t1 $\longrightarrow$ t2, where t1 and t2 are both terms of the grammar defined in \Cref{lst:ottgrammarex}. There are three statements for function application: the actual substitution, reduction of the first term and reduction of the second term. 
...
In previous subsections there were no constructs that accept external computations and MOCaml could not behave as a general purpose concurrency framework without the ability to insert arbitrary computations. In \Cref{fig:syntypcomp} I introduce a new expression called a computation place holder. These are the holes that will be filled by actual OCaml $ \textbf{unit} \rightarrow \text{expr} $ functions to be evaluated. 
...
Bind evaluates left to right: it evalutes the expression on the left to a concurrent box with rule (\ref{eq:r-evalbind}), reduces the internal expression of the box with rule (\ref{eq:r-movebind}) and then silently passes it to the expression on the right with rule (\ref{eq:r-dobind}). This fits well with the idea of sequencing operations: any side effect $ e' $ might have happens after the side effects of $ e $.
% Fork opsem
    ...
    \begin{minipage}{\linewidth}
     \begin{tabularx}{\textwidth}{>{\raggedright}X}
                 \textit{Evaluation} \hfill \fbox{$e \, \red{s}{rl} \, e'$}  \tabularnewline   \begin{equation}
                        ...
                        \tabularnewline\begin{equation}\inferrule{e' \red{s}{rl} e'' }{ \textbf{fork }\,\,  v\,\, e'  \red{s}{rl} \textbf{fork } \,\, v \,\, e'' } \tag{R-Forkeval2} \label{eq:forkeval2}\end{equation}   
                 \end{tabularx}
    \end{minipage}
    \end{tabular}
}}}$
...
The various equivalences of \Cref{sec:equivs} give meaningful insight into the behaviour of the mentioned expressions when considered in isolation. However, it falls short of the original aim of provide an equality that implies the ability to replace these expressions when composed with others. 
...
Construction of a congruence relation over MOCaml is desirable, however not at all trivial. It is easy to see that the previously defined value equal weak bisimilarity (\Cref{defn:ve_weak_bisimilarity}) is not a congruence: $ \lambda $-abstractions are only  value equal weak bisimilar to themselves, as they are values. Even though the body may be substituted, the $ \lambda $-abstraction would not be equivalent under this relation.
