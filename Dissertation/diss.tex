...
\Cref{fig:evalmon} shows the semantics of the concurrent type constructor \textbf{ret} and the operation \textbf{bind}. The \textbf{ret} operation first reduces its argument to a value with rule (\ref{eq:r-evalret}) and then boxes it up silently as a concurrent value with rule (\ref{eq:r-ret}). 
...
A monadic concurrency framework should satisfy a number of properties: monadic laws, process calculus axioms and potentially properties of the fixpoint operator, predicates for pairs, type preservation and progress. These requirements are all phrased as equivalence between expressions as mentioned in \Cref{sec:first_mention_of_equiv}. 
...
First, I describe the three monadic laws mentioned in \Cref{sec:monad_first_mention}. Then I attempt to show some of the basic process algebra axioms used by Bergstra for his Algebra of Communicating Processes\cite{bergstra1984process,bergstra1986algebra}. These properties can be summarized as follows:
\begin{align}
...
If we instead interpret $ + $ as the fork primitive of MOCaml only \Cref{eq:fork_comm,eq:fork_assoc,eq:fork_deadlock} may hold. Due to the semantics of \textbf{fork} \Cref{eq:choice_id,eq:fork_bind_distribute} do not hold as they may duplicate side-effects. Sequential composition or bind is denoted by $ \cdot $.
