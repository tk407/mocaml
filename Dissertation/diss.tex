...
\usepackage{tikz}
\usepackage{tikz-cd}
\usetikzlibrary{arrows, topaths, shapes, positioning, fit, trees, decorations.pathmorphing, decorations.markings}
%\usepackage{harvard}
...
by {\tt texcount, version 3.0.0.24}
}
...
%\label{sec:monad_first_mention}A monad\cite{hoareetal2001tackling} in functional programming is a construct to structure computations that are in some sense ``sequenced" together. This sequencing can be for example string concatenation, simple operation sequencing (the well known semicolon of imperative programming) or conditional execution. Two operations commonly called bind and return and a type constructor of a parametric type, for example $ \textbf{con} \, T $ where $ T $ is any type, form a monad when they obey a set of axioms called monadic laws.
%
%Most monads support further operations and a concurrency monad is one such monad. Beside the two necessary operations (return and bind) a concurrency monad has to support at least one that deals with concurrent execution.
...
I will return to the exact nature of the equivalence relation $ \equiv $ used in this project in the theoretical evaluation section(\Cref{sec:behave_equiv}). \label{sec:first_mention_of_equiv}
...
In \Cref{lst:ottgrammarex} the non-terminal t for terms is defined with 5 productions: variables, lambda abstractions, applications, parentheses grouping and variable substitution. Each of these rules have a name, such as Var and Lam. Each of these names are prefixed by the string ``t\_" in translation to have non-ambiguous names. The right hand side of each line in \Cref{lst:ottgrammarex} is a so-called homomorphism. A homomorphism describes a manual translation to a target language.  \label{sec:ott_homo} It starts with a destination like \lstinline[language={Ott}]|ocaml| for OCaml,\lstinline[language={Ott}]|tex| or \lstinline[language={Ott}]|com| for \LaTeX\, and \lstinline[language={Ott}]|ich| for Isabelle, Coq and HOL. An expression in double square brackets \lstinline[language={Ott}]|[[t]]| stands for the translation of the expression within the brackets. 
...
The variable \lstinline[language={Ott}]|x| in \Cref{lst:ottgrammarex} is a metavariable. Metavariables used in productions are defined with their destination language equivalents and potentially (in the case of Coq) their equality operation.
...
The statement \lstinline[language={Ott}]|single t x :: tsubst| in \Cref{lst:ottsubstex} defines a single substitution function called tsubst\_t over terms defined by the grammar for t and for variables represented by the metavariable x. This is the same tsubst\_t mentioned \Cref{lst:ottgrammarex}.
...
In \Cref{lst:ottredex} I define a set of mutually recursive relations named \verb|Jop| with one relation in it, the $\longrightarrow$ or reduction relation. Each element of this relation takes the form t1 $\longrightarrow$ t2, where t1 and t2 are both terms of the grammar defined in \Cref{lst:ottgrammarex}. There are three statements for function application: the actual substitution, reduction of the first term and reduction of the second term. 
...
Coq can define predicates and relations over sets. In \Cref{lst:coqpredex} I have defined a proposition, that if the product of two integers is zero, then at least one of them must be zero.
...
Data structures can also be defined both inductively(\Cref{lst:coqinddataex}) and co-inductively(\Cref{lst:coqcoinddataex}).\label{sec:coq_coind} Co-inductive data structures represent potentially infinite data.  They are supported by a number of programming languages, since they are useful for defining data structures such as lazy lists, trees and streams.
...
\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=2cm, auto, >=stealth',pil/.style={
           ->,
           thick,
           shorten <=2pt,
           shorten >=2pt,}, pildashed/.style={
                      ...
                      dashed,
                      shorten <=2pt,
                      shorten >=2pt,},scale=1.4, every node/.style={scale=0.9}, ]
\node[draw, rectangle] (o) {Ott};
\node[right of=o] (otch) {};
\node[below of={otch}] (otc) {Generation}
  edge[pildashed] (otch);
\node[draw, rectangle, right of={otch}] (c) {Coq}
  edge[pil, <-] (o);
\node[right of=c] (ceoh) {};
\node[below of={ceoh}] (ceo) {Hand modification}
  edge[pildashed] (ceoh);
\node[draw, rectangle, right of={ceoh},align=center, text width=2.1cm] (exc) {Extractable Coq}
  edge[pil, <-] (c);
\node[right of=exc] (ceoh) {};
\node[below of={ceoh}] (ceo) {Extraction}
  edge[pildashed] (ceoh);
\node[draw, rectangle, right of={ceoh}] (oc) {OCaml}
  edge[pildashed, <-] (exc);
\node[draw, ellipse, above of={ceoh}] (exp) {Extraction plugin}
  edge[pil, <-] (exc)
  edge[pil] (oc);
\node[right of=oc] (rceoh) {};
\node[draw, rectangle, right of={rceoh},align=center, text width=1.65cm] (roc) {Runnable OCaml}
  edge[pil, <-] (oc);
\node[below of={rceoh}] (rceo) {Hand modification}
  edge[pildashed] (rceoh);
\end{tikzpicture}
...
\item[Stage 3.]{I manually modified the Coq representation to be suitable for the extraction plugin detailed in \Cref{sec:first_mention_logind}.}
...
\begin{figure}[h]
  ...
  \begin{tabular}{l | l}
    ...
    \end{tabularx} 
    \end{minipage} & \begin{minipage}{0.63\linewidth}
        \begin{tabular}{l}
        \begin{minipage}{\linewidth}
           \begin{tabularx}{\linewidth}{>{\raggedright}X}
                        ...
                        { \Gamma\, \vdash \, (\textrm{\textbf{case} $  e $ \textbf{of} \newline \textbf{left} $ x_1 \Rightarrow e_1 $  \vline\, \textbf{right} $ x_2 \Rightarrow e_2 $ })\, :\, T''} \tag{T-Case}  \label{eq:t-case}
                                                  \end{equation}\vspace{-5mm}
                      \end{tabularx}
        \end{minipage}
        \end{tabular}
...
Many circumstances require the ability to describe expressions that are either one type or another. Sum types or otherwise known as labelled unions are a simple solution to this. 
...
In \Cref{lst:coqselectstar} I define \verb|select| with the co-inductive notation introduced in \Cref{sec:coq_coind}, where each element is a pair of a decision and a further \verb|select| representing the rest of the decisions. 

The 4-tuple $ (e, s, rl, e') $ means that given the selection operator $ s $, a starting expression $ e $  can move to expression $ e' $ with a side-effect $ rl $. I use the notation $ e\, \red{s}{rl}\, e' $ as in (\ref{eq:red}) in \Cref{fig:reductiom_semantics}. The $ rl $ label may be $ \tau $ which is a silent action or an atomic action $ e $ that can be observed. A silent action $\tau$ is considered unobservable from outside MOCaml. The atomic action $e$ is considered an expression within the model.  This choice was based on the behaviour of placeholders in \Cref{sec:comp_place_syn}. The inspiration to use a labelled transition system to model side-effects comes from Milner's ``A calculus of communicating systems" book\cite{milner1982calculus}.
   ...
   \begin{tabularx}{\textwidth}{l l r}
           ...
           {\rule{\linewidth}{\arrayrulewidth}}\vspace{2mm}\\
            \begin{tabularx}{\textwidth}{>{\raggedright}X}
                ...
                \inferrule
                { e\, \red{s}{rl}\, e'}
                { \textbf{left}\,e \, \red{s}{rl}\, \textbf{left}\,e'\,  } \tag{R-Left} \label{eq:r-left} \end{equation}\vspace{-5mm}
                \tabularnewline
                \begin{equation}
                \inferrule
                { e\, \red{s}{rl}\, e'}
                { \textbf{right}\,e \, \red{s}{rl}\, \textbf{right}\,e'\,  } \tag{R-Right}\label{eq:r-right} \end{equation}\vspace{-5mm} \tabularnewline  
                \begin{equation}
                        \inferrule
                                                       { e\, \red{s}{rl}\, e' }
                                                       { (\textrm{\textbf{case} $  e $ \textbf{of} \textbf{left} $ x_1 \Rightarrow e_1 $ \vline\, \textbf{right} $ x_2 \Rightarrow e_2 $ }) \\\\ {\scriptstyle s}\!\big\downarrow\!{\scriptstyle rl} \\\\
                           ...
                           \begin{equation}
                                        \inferrule
                                          {  }
                                          { (\textrm{\textbf{case} $  \textbf{left}\, v $ \textbf{of} \textbf{left} $ x_1 \Rightarrow e_1 $ \vline\, \textbf{right} $ x_2 \Rightarrow e_2 $ })\, \red{s}{\tau}\, \{v / x_1\}e_1 } \tag{R-CaseL}\label{eq:r-casel}
                                        \end{equation}  \vspace{-11mm}
                                        \tabularnewline \begin{equation}
                                            \inferrule
                                            {  }
                                            { (\textrm{\textbf{case} $  \textbf{right}\, v $ \textbf{of} \textbf{left} $ x_1 \Rightarrow e_1 $ \vline\, \textbf{right} $ x_2 \Rightarrow e_2 $ })\, \red{s}{\tau}\, \{v / x_2\}e_2 } \tag{R-CaseR} \label{eq:r-caser}
                                                                             \end{equation}\vspace{-5mm}
                \end{tabularx}
           \end{minipage}
}}}$
...
In \Cref{fig:semeval} I describe the transition rules for constructs related to sum types: the destructor \textbf{case} and the constructors \textbf{left} and \textbf{right}. The tags \textbf{left} and \textbf{right} reduce their argument until it is a value by rules (\ref{eq:r-left}) and (\ref{eq:r-right}) respectively. The \textbf{case} expression first reduces its argument $ e $ to a tagged value by rule (\ref{eq:r-case}). Depending on the tag of the value, this is then silently reduced by either (\ref{eq:r-casel}) or (\ref{eq:r-caser}) to  $ e_1 $ or $ e_2 $ respectively with substituting the value within the tag for the corresponding variable. The \textbf{case} expression can be used to dynamically exploit some behaviour information and signal within MOCaml without the need to include the complexity of redefining conditional constructs like \textbf{if}. Much like in the case of functions, \textbf{case} binds the variables $ x_1 $ and $ x_2 $ within the expressions $ e_1 $ and $ e_2 $ respectively. As the substitution does not avoid variable capture the user must take care to use fresh variables. 
...
\begin{figure}[H]
  ...
  \begin{tabularx}{\textwidth}{l l r}
                      ...
                      \end{tabularx}\\
                  {\rule{1\linewidth}{\arrayrulewidth}}\\
  \begin{tabular}{l  l}
    \begin{minipage}{0.42\linewidth}
     \vspace{0mm}\begin{tabular}{l}
    \begin{tabularx}{\textwidth}{>{\raggedright}X}
     \textit{Evaluation} \hfill   \tabularnewline   \begin{equation}
                                         \inferrule
                                           { e\, \red{s}{rl}\, e''}
                                           { \{e,e'\}\, \red{s}{rl}\, \{e'',e'\} } \tag{R-Pair1}\label{eq:r-pair1}
                                         \end{equation} \vspace{-5mm}
                                         \tabularnewline
                                         \begin{equation}
                                           \inferrule
                                             { e'\, \red{s}{rl}\, e''}
                                             { \{v,e'\}\, \red{s}{rl}\, \{v,e''\} } \tag{R-Pair2} \label{eq:r-pair2}
                                             \end{equation} \vspace{-5mm}
                                         \tabularnewline
                                          \begin{equation}
                                         \inferrule
                                         {}
                                         { \textbf{proj}_1\,\{v,\,\,v'\} \, \red{s}{\tau}\, v  } \tag{R-Proj1}\label{eq:r-proj1}
                                         \end{equation}\vspace{-7mm}
    \end{tabularx} 
    \end{tabular}
    \end{minipage} & \begin{minipage}{0.43\linewidth}
        \begin{tabular}{l}
        \begin{minipage}{\textwidth}
         \begin{tabularx}{\textwidth}{>{\raggedright}X}
          \vspace{2.5pt} \hfill \fbox{$e \, \red{s}{rl} \, e'$}\tabularnewline \vspace{1pt}
                                     \begin{equation}
                                       \inferrule
                                       { e\, \red{s}{rl}\, e'}
                                       { \textbf{proj}_1\,e \, \red{s}{rl}\, \textbf{proj}_1\,e'  } \tag{R-Proj1-Eval}\label{eq:r-evalproj1}
                                       \end{equation}\vspace{-7mm}
                                                     \tabularnewline
                                     \begin{equation}
                                     \inferrule
                                     { e\, \red{s}{rl}\, e'}
                                      { \textbf{proj}_2\,e \, \red{s}{rl}\, \textbf{proj}_2\,e'  } \tag{R-Proj2-Eval}\label{eq:r-evalproj2}
                                      \end{equation}\vspace{-7mm}
                                     \tabularnewline
                                      \begin{equation}
                                       \inferrule
                                      {}
                                        { \textbf{proj}_2\,\{v,\,\,v'\} \, \red{s}{\tau}\, v'  } \tag{R-Proj2}\label{eq:r-proj2}
                                                                                                                \end{equation}
             \end{tabularx}
        \end{minipage} 
        \end{tabular}
...
In previous subsections there were no constructs that accept external computations and MOCaml could not behave as a general purpose concurrency framework without the ability to insert arbitrary computations. In \Cref{fig:syntypcomp} I introduce a new expression called a computation placeholder. This is a hole that will be filled by an actual OCaml $ \textbf{unit} \rightarrow \text{expr} $ function to be evaluated. 
...
\begin{itemize}
\item{ret, also known as return. From the monadic axioms, mentioned in \Cref{sec:monad_second_mention}, it follows that it has type $ T \rightarrow \textbf{con }T   $. This type corresponds to the fully applied typing rule (\ref{eq:t-ret}) in \Cref{fig:syntypmon}}.
...
\Cref{fig:evalcomp} shows the reduction rule (\ref{eq:r-evalcomp}) for a computation placeholder. It only exists in this form in the logical model: the placeholder will be fully replaced by a call to an OCaml function that returns an expression. A simple example could be \Cref{lst:ocaml_comp_place}. This computation placeholder is then reduced by the case in \Cref{lst:ocaml_red_comp}. Computation placeholders serve as a tool to prove, in \Cref{sec:behave_equiv}, that the sequence of actual code calls obey certain order properties. 
...
I chose to always reduce a placeholder to an expression within the framework to provide an easy way for flexible, dynamic behaviour. This also means that the typing statement in \Cref{fig:syntypcomp} is not enforced: it is assumed that the user pays attention to the type of the returned expression. Future work could include a way to type check expressions and to modify the extracted OCaml to enforce this typing rule.
...
\begin{figure}[H]
  ...
  $\underbracket{\overbracket{\textrm{
  \begin{tabular}{l}
  \begin{tabularx}{0.7\linewidth}{l l r}
                        \textit{Values} &  & \tabularnewline
                          $ v $\, ::=  &\dots  & \textit{values:}\tabularnewline
                          & $ \textbf{live } e$  & \textit{live}\tabularnewline
                        \end{tabularx}\\
                    {\rule{0.9\linewidth}{\arrayrulewidth}}\\
 \begin{minipage}{0.9\textwidth}
          \begin{tabularx}{\textwidth}{>{\raggedright}X}
              \textit{Evaluation} \hfill \fbox{$e \, \red{s}{rl}\, e'$}  \tabularnewline    {\begin{equation}
                                       \inferrule
                                        {}
                                        { \textbf{ret }\,v \, \red{s}{\tau}\, \textbf{live } v } \tag{R-Return}\label{eq:r-ret}
                                        \end{equation}\vspace{-9mm}}
                                        \tabularnewline   { \begin{equation}
                                        \inferrule
                                         { e\, \red{s}{rl}\, e'}
                                        { \textbf{ret }\,e \, \red{s}{rl}\, \textbf{ret }\,e' } \tag{R-Evalret}\label{eq:r-evalret}
                                                                                \end{equation}\vspace{-7mm}}
                                      \tabularnewline  { \begin{equation}
                                                                            \inferrule
                                                                            {}
                                                                            { \textbf{live }  v\,\gg=\,e' \, \red{s}{\tau}\, e'\, v  } \tag{R-Dobind}\label{eq:r-dobind} \end{equation}\vspace{-9mm}}
                                      \tabularnewline  { \begin{equation}
                                      \inferrule
                                       { e\, \red{s}{rl}\, e''}
                                       { \textbf{live }  e\,\gg=\,e' \, \red{s}{rl}\, \textbf{live } e''\, \gg= e'\,  } \tag{R-Movebind}\label{eq:r-movebind}
                                                                \end{equation}\vspace{-7mm}}
              \tabularnewline  { \begin{equation}
              \inferrule
              { e\, \red{s}{rl}\, e''}
              { e\,\gg=\,e' \, \red{s}{rl}\, e''\, \gg= e'\,  } \tag{R-Evalbind} \label{eq:r-evalbind}
              \end{equation}\vspace{-5mm}}
              \end{tabularx}
         \end{minipage}
         \end{tabular}
...
\Cref{fig:evalmon} shows the operational semantics of the concurrent type constructor \textbf{ret} and the operation \textbf{bind}. The \textbf{ret} operation first reduces its argument to a value with rule (\ref{eq:r-evalret}) and then boxes it up silently as a concurrent value with rule (\ref{eq:r-ret}). 


Bind evaluates left to right: it evaluates the expression on the left to a concurrent box with rule (\ref{eq:r-evalbind}), reduces the internal expression of the box with rule (\ref{eq:r-movebind}) and then silently passes it to the expression on the right with rule (\ref{eq:r-dobind}). This fits well with the idea of sequencing operations: any side effect $ e' $ might produce happens after the side effects of $ e $.
% Fork opsem
    ...
    \begin{minipage}{\linewidth}
     \begin{tabularx}{\textwidth}{>{\raggedright}X}
                 \textit{Evaluation} \hfill \fbox{$e \, \red{s}{rl} \, e'$}  \tabularnewline   \begin{equation}
                                         \inferrule
                                           {e\, \red{s}{el}\, e'' \\ e' \notin \text{Values} }
                                           { \textbf{fork } (\textbf{live } e) (\textbf{live } e')\,  \red{L\,\,s}{rl}\, \textbf{fork } (\textbf{live } e'') (\textbf{live } e')  } \tag{R-Forkmove1} \label{eq:forkmove1}
                                         \end{equation} \vspace{-7mm}
                                         \tabularnewline   \begin{equation}
                                          \inferrule
                                            {e'\, \red{s}{rl}\, e'' \\ e \notin \text{Values} }
                                            { \textbf{fork } (\textbf{live } e) (\textbf{live } e')\, \red{R\,\,s}{rl}\, \textbf{fork }  (\textbf{live } e) (\textbf{live } e'')  } \tag{R-Forkmove2} \label{eq:forkmove2}
                                           \end{equation}\vspace{-7mm}\tabularnewline
                                            \begin{equation}
                                            \inferrule
                                            {}
                                            { \textbf{fork } (\textbf{live } v) (\textbf{live } e')\,  \red{s}{\tau}\, \textbf{live } \textbf{left}  \{v, (\textbf{live } e')\}  } \tag{R-Forkdeath1}\label{eq:r-forkdeath1}   \end{equation} \vspace{-7mm} \tabularnewline
                          \begin{equation}
                                                                      \inferrule
                                                                      {}
                         { \textbf{fork } (\textbf{live } e) (\textbf{live } v')\,  \red{s}{\tau}\, \textbf{live } \textbf{right}  \{(\textbf{live }e) ,  v'\}  } \tag{R-Forkdeath2}\label{eq:r-forkdeath2}   \end{equation} \vspace{-7mm}\tabularnewline\begin{equation}\inferrule{e\, \red{s}{rl}\, e'' }{ \textbf{fork }\,\,  e\,\, e'\, \red{s}{rl}\, \textbf{fork } \,\, e'' \,\, e' } \tag{R-Forkeval1} \label{eq:forkeval1}\end{equation}\vspace{-7mm}
                        \tabularnewline\begin{equation}\inferrule{e'\, \red{s}{rl}\, e'' }{ \textbf{fork }\,\,  v\,\, e'\,  \red{s}{rl}\, \textbf{fork } \,\, v \,\, e'' } \tag{R-Forkeval2} \label{eq:forkeval2}\end{equation}\vspace{-5mm}   
                 \end{tabularx}
    \end{minipage}
    \end{tabular}
}}}$
...
In \Cref{fig:evalfork} I give the detailed operational semantics of \textbf{fork}. First, \textbf{fork} reduces its arguments, left-to-right, to \textbf{live} boxes by rule (\ref{eq:forkeval1}) and then by rule (\ref{eq:forkeval2}). This is the first time the selection operator of the reduction relation is explicitly specified. If neither edge is a value, decision $ L $ reduces the left edge with rule (\ref{eq:forkmove1}) while a decision $ R $ reduces the right edge with rule (\ref{eq:forkmove2}). When either edge contains a value, \textbf{fork} is forced to silently reduce to the respective tagged value with rules (\ref{eq:r-forkdeath1}) and (\ref{eq:r-forkdeath2}), regardless of the decision. This gives a finer grade of control for the user to choose what to do with the partially reduced edge, however it increases overhead as value checks always have to be carried out on both edges.   \clearpage
...
\begin{enumerate}
\item{\textbf{Inductive sets} are the inductive data structures of Coq. These were translated from expression grammar objects. As an example, type expressions were translated from the Ott version in \Cref{lst:otttypexpr} to the Coq equivalent in \Cref{lst:coqtypexpr}.
...
Grammar terms have a one-to-one correspondence except for $ (\,t\,) $ that has no counterpart: the flag S tells Ott that it is only syntactic sugar. Syntactic sugar, as explained in \Cref{sec:ott_homo}, does not get translated to a type constructor, but uses the instruction in the homomorphism:
...
\end{minipage}	
Ott offers single and multiple substitution predicates for its destination languages. These are implemented as fixpoints in Coq.  Expression substitution (\Cref{lst:coqexprsubst}) is an example of these automatically generated fixpoints.
\begin{minipage}{\linewidth}
...
\item{\textbf{Logical inductive sets} are inductively defined sets of propositions. The reduction relation and the typing relation is represented as logical inductive sets. For example a clause in the reduction relation was translated from the simple inference rule presentation in \Cref{lst:ottcontextapp1} to a proposition in the logical inductive set \lstinline[language={Coq}]|JO_red| in \Cref{lst:coqlogind}.
...
As I mentioned in \Cref{sec:first_mention_logind}, Coq provides built-in extraction facilities to OCaml and Haskell, but not from inductive sets and fixpoints that involve propositions, more specifically the \textit{Prop} sort. However, with the extraction plugin developed by Delahaye et al\cite{delahaye2007extracting,tollitte2012producing} I can extract such programs.
...
Tollitte\cite{tollitte2012producing} made progress on merging cases when one case subsumes another. However, in the case of MOCaml, these were not always correctly identified. For example the case in \Crefrange{lst:ocamlvalexpappcaseStart}{lst:ocamlvalexpappcaseEnd} in \Cref{lst:ocamlorigapp} subsumes the case in \Cref{lst:ocamlorigsub}. The plugin was unable to infer this as that would have required it to show that function term is always a value, regardless of its arguments. This is an issue as any function applied to an argument that can be reduced will fail, as the case in \Cref{lst:ocamlorigsub} comes first. 
...
\end{minipage}
A reoccurring inefficiency comes from the extraction plugin generating a default failing case in all situations, even if the default case may never happen. \Cref{lst:ocamlunneededdefault2} in \Cref{lst:ocamlorigapp} is an example of an unnecessary default case: a boolean may only take the values true or false. A similar issue occurs when evaluating a function:  \Cref{lst:ocamlunneededmatchonfun} in the same listing matches by giving a name to the return value, but generates a default case as well. I simply removed the superfluous match and replaced the name of the return value with the function call.
...
\item{Round-robin and random schedulers in \Cref{lst:ocamlrrsched} and \Cref{lst:ocamlrandsched} respectively.\label{sec:random_sched}}
\end{itemize}
\begin{minipage}{\linewidth}
...
A monadic concurrency framework should satisfy a number of properties: monadic laws, process calculus axioms. These requirements are all phrased as equivalence between expressions as mentioned in \Cref{sec:first_mention_of_equiv}. 
...
\Cref{lst:takeright_op} shows the definition of the $ \mathit{takeright} $ function in a simplified notation. \Cref{lst:orig_coq_takeright} shows the original Coq definition. Much like $\mathit{swapf}$, $\mathit{takeright}$ is also a family of functions based on the return type $ T $ of the preceding \textbf{fork}.
...
The various equivalences of \Cref{sec:equivs} give meaningful insight into the behaviour of the mentioned expressions when considered in isolation. However, it falls short of the original aim of providing an equality that implies the ability to replace these expressions when composed with others. 
...
Construction of a congruence relation over MOCaml is desirable, however not at all trivial. It is easy to see that the previously defined value equal weak bisimilarity (\Cref{defn:ve_weak_bisimilarity}) is not a congruence: The body of a $ \lambda $-abstraction is not a context that is compositional. As $ \lambda $-abstractions are values, two $ \lambda $-abstractions are only equivalent under value equal weak bisimilarity if their bodies are textually equal.
...
In \Cref{fig:kpn_process_outline,fig:sieve_process_outline,fig:sorter_process_outline} processes are denoted with a circle. Message variables and FIFOs are denoted by squares. Solid lines denote data flow and dashed lines mean process creation.
\begin{enumerate}
...
\centering
\scalebox{1}{%
\hspace*{-3em}\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
  thick,main node/.style={circle,draw,
  font=\sffamily\Large\bfseries,minimum size=8mm}, mvar/.style={rectangle, draw, ,minimum size=3mm}]
  \begin{scope}
  \clip (-2,-5) rectangle (14.25, 4.3);
  ...
  \node[main node] (T2) [right = 1 of F2, label=below:{$\times \textit{2}$}] {};
  \node[main node] (T3) [right = 1 of F3,  label=below:{$\times \textit{3}$}] {};
  \node[main node] (T5) [right = 1 of F5, label=below:{$\times \textit{5}$}] {};
  ...
  \node[main node] (Merge35) [below=0.5 of M3, xshift=2cm, label=below:{$\textit{Merge}_{35}$}] {};
  \node[mvar] (M35) [right of=Merge35, label=below:{$\textit{m}_{35}$}] {};
  \node[main node] (Merge235) [below=0.5 of M2, xshift=6cm, label=below:{$\textit{Merge}_{235}$}] {};
   \node[mvar] (M235) [right of=Merge235, label=below:{$\textit{m}_{235}$}] {};
  \node[main node] (P) [right of=M235, label=below:{\textit{P}}] {};
...
%  \node[main node] (I) [right of=F] {I};
  ...
  \path[every node/.style={font=\sffamily\small,
    ...
    (Merge235) edge   (M235)
    (M235) edge   (P)
    (P) edge [out=90, in=135, distance=3.5cm] (F2)
    (P) edge [out=90, in=135, distance=7cm]  (F3)
    (P) edge [out=90, in=135, distance=9.5cm]  (F5);
  \end{scope}
\end{tikzpicture}}
...
\item{\textit{Sift}: If a number $ p $ is read \textit{Sift} puts $ p $ forward to the printer process. \textit{Sift} also creates a new process, \textit{Filter p}  and inserts it between the input of \textit{Sift} and \textit{Sift} itself in the flow of numbers (\Cref{fig:sieve_process_outline}). }
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
  thick,main node/.style={circle,draw,
  font=\sffamily\Large\bfseries,minimum size=8mm}, mvar/.style={rectangle, draw, ,minimum size=3mm}]
  \node[main node] (I1) [label=below:{\textit{Integers}}] {};
  \node[mvar] (M11) [right=0.5 of I1, label=below:{$\text{m}_1$}] {};
  \node[main node] (S1) [right=8 of M11, label=below:{\textit{Sift}}] {};
  \node[mvar] (MS1) [right=0.5 of S1, label=below:{$\text{m}_s$}] {};
  \node[main node] (P1) [right=1 of MS1, label=below:{\textit{Print}}] {};
 
  \node[main node] (I2) [below= of I1, label=below:{\textit{Integers}}] {};
  \node[mvar] (M12) [below=2.5 of M11, label=below:{$\text{m}_1$}] {};
  \node[main node] (F2) [right=0.5 of M12, label=below:{\textit{Filter 2}}] {};
  \node[mvar] (M22) [right=0.5 of F2, label=below:{$\text{m}_s$}] {};
  \node[main node] (S2) [below= of S1, label=below:{\textit{Sift}}] {};
  \node[mvar] (MS2) [below=2.5 of MS1, label=below:{$\text{m}_s$}] {};
  \node[main node] (P2) [below= of P1, label=below:{\textit{Print}}] {};
  \node[main node] (I3) [below= of I2, label=below:{\textit{Integers}}] {};
    \node[mvar] (M13) [below=2.5 of M12, label=below:{$\text{m}_1$}] {};
    \node[main node] (F22) [right=0.5 of M13, label=below:{\textit{Filter 2}}] {};
      \node[mvar] (M23) [right=0.5 of F22, label=below:{$\text{m}_2$}] {};
   \node[main node] (F5) [right=0.5 of M23, label=below:{\textit{Filter 3}}] {};
     \node[mvar] (M33) [right=0.5 of F5, label=below:{$\text{m}_s$}] {};
    \node[main node] (S3) [below= of S2, label=below:{\textit{Sift}}] {};
    \node[mvar] (MS3) [below=2.5 of MS2, label=below:{$\text{m}_s$}] {};
    \node[main node] (P3) [below= of P2, label=below:{\textit{Print}}] {};
  %\node[mvar] (F3) [below of=F2, label=below:{$\text{FIFO}_3$}] {};
  %\node[mvar] (F5) [below of=F3, label=below:{$\text{FIFO}_5$}] {};
...
As described in \Cref{sec:random_sched}, the default MOCaml scheduler chooses threads to reduce at random. Unexpectedly, the default implementation (MOCaml) in \Cref{fig:sieve_exec_times} exhibits an exponential behaviour. The reason for this is in the way random selection parameters relate to the \textbf{fork} tree formed by the expression.
...
The last example is concurrent sort. Sorting of a list of integers can be implemented by single comparator units which take in two values and output the lower on one edge and the higher on the other (\Cref{fig:sorter_process_single}). To sort an entire list we can arrange these elements in a network shown in \Cref{fig:sorter_process_full}. This network takes in the $ n $ element list $ x_0, x_1, \dots, x_n $ and outputs the result $ r_0, r_1, \dots, r_n $. For an $ n $ element list the network has $ \frac{n(n-1)}{2} $ nodes. Different schedulings of this network correspond to different algorithms, such as bubble sort and insertion sort. For simplicity I did not show the message variables in \Cref{fig:sorter_process_outline} but they are used on each arrow. This problem is a good example of a task best suited for lightweight concurrency: a very large number of simple threads statically allocated. For a list size of 500, there are 124750 threads. 


As expected, my system performed with the same characteristics as other lightweight concurrency solutions, but with a high overhead. Interestingly, the \textbf{vm} lightweight implementation performs very poorly, even though it is lightweight. On the other hand, the heavyweight \textbf{sys} implementation changes characteristics when the list grows larger than 250 elements and performs similarly to lightweight implementations. The memory use of my system is much higher than the other implementations (\Cref{fig:sorter_mem}).
